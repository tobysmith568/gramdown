...
**Already using a package manager?** macOS: skip ahead to [Homebrew](#already-using-homebrew). Windows: WinGet support is on its way. Until then, npm or the binary above both work.
...
If you're on macOS with Homebrew installed, gramdown is available from [the tobysmith568 tap](https://github.com/tobysmith568/homebrew-tap):

```sh
brew install tobysmith568/tap/gramdown
```

Supports both Apple Silicon and Intel. Update it the same way as any other formula: `brew upgrade`.
